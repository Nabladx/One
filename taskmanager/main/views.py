...
def index(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            count_test_data = sum([i.strip(string.punctuation).isalpha() for i in test_data.split()])
            ...
            train = [[NOUN_train], [ADP_train], [ADJ_train],
                    ...
                    [PROPN_train_root], [PROPN_train_conj], [PROPN_train_iobj],
            ...
            if results < 0.87:
                ...
                return HttpResponse('{} "{}" <br>Точность принадлежности равна <ins>{}</ins>%'.format(html, name, results))
            else:
                results *= 10000
                results = (int(results)) / 100.0
